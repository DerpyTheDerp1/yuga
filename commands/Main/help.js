...
exports.run = async(client, msg, args) => {
  ...
  if (cmdName) {
    ...
    const ExclusiveCommands = fs.readdirSync(__dirname + '/Exclusive').map(file => path.basename(file, path.extname(file)));
    const FunCommands = fs.readdirSync(__dirname + '/Fun').map(file => path.basename(file, path.extname(file)));
    const MainCommands = fs.readdirSync(__dirname + '/Main').map(file => path.basename(file, path.extname(file)));
    const ModerationCommands = fs.readdirSync(__dirname + '/Moderation').map(file => path.basename(file, path.extname(file)));
    const UtilityCommands = fs.readdirSync(__dirname + '/Utility').map(file => path.basename(file, path.extname(file)));
